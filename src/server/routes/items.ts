...
function sortItems(items: Item[]) {
  return [...items].sort((left, right) => {
    ...
    return left.startsAt - right.startsAt || priorityRank[right.priority] - priorityRank[left.priority] || left.createdAt - right.createdAt;
  });
}
...
export async function registerItemRoutes(app: FastifyInstance) {
  ...
  app.patch("/items/:id", async (request, reply) => {
    ...
    const updated = await db.query.itemsTable.findFirst({
      where: eq(itemsTable.id, params.id),
    });
  ...
  app.post("/items/:id/skip-next", async (request, reply) => {
    ...
    const rule = new RRule({
      ...options,
      dtstart: new Date(existing.startsAt * 1000),
    });
    ...
    const exceptions = Array.from(new Set([...parseExceptions(existing.exceptions), Math.floor(nextOccurrence.getTime() / 1000)])).sort(
