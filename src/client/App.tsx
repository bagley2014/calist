...
export default function App() {
	...
	async function bootstrap() {
		try {
			const setup = await apiRequest<{ isConfigured: boolean }>('/api/setup/status', { headers: {} });
	...
	async function deleteItem(itemId: string) {
		await apiRequest<void>(`/api/items/${itemId}`, { method: 'DELETE' });
	...
	async function regenerateIcsKey() {
		const nextSettings = await apiRequest<Pick<SchedulerSettings, 'apiKey' | 'exportUrl'>>(
			'/api/settings/ics-key/regenerate',
			{ method: 'POST' }
		);

		setSettings((current) =>
			current ? { ...current, apiKey: nextSettings.apiKey, exportUrl: nextSettings.exportUrl } : null
		);
