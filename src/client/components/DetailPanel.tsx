  ...
  return (
    <aside className="detail-panel">
      ...
      <div className="detail-panel__body">
        ...
        <label>
          Recurrence rule
          <textarea rows={3} value={rrule} onChange={(event) => setRRule(event.target.value)} placeholder="FREQ=WEEKLY;BYDAY=MO" />
        </label>
      ...
      <div className="detail-panel__actions">
        ...
        <button type="button" className="button button--ghost" disabled={busy} onClick={() => runAction(() => onToggleComplete(item, !item.completed))}>
          {item.completed ? "Mark active" : "Mark complete"}
        </button>
        {item.rrule ? (
          <button type="button" className="button button--ghost" disabled={busy} onClick={() => runAction(() => onSkipNext(item.id))}>
